...
We've tested how the model performs under block bootstrapping, now we will perform GARCH Monte Carlo simulations and test how how model fits that data and performs.
...
fit <- FitML(spec = spec1, data = sp500_df$Log_Returns)

n_sim <- 2       # Number of simulations
n_ahead <- nrow(sp500_df)      # Simulation horizon (10 days)
nburn <- 100        # Burn-in period

# Simulate paths
set.seed(123)  # Ensure reproducibility
sim <- simulate(object = fit, nsim = n_sim, nahead = n_ahead, nburn = nburn)

# Extract simulated data
simulated_returns <- sim$draw      # Simulated returns
simulated_volatility <- sim$CondVol  # Conditional volatilities
simulated_states <- sim$state      # Simulated states (if Markov-switching)

n_ahead <- nrow(simulated_returns)
n_sim <- ncol(simulated_returns)

# Convert simulation data into tidy format for ggplot
sim_returns_tidy <- as.data.frame(simulated_returns) %>%
  mutate(Day = 1:n_ahead) %>%
  pivot_longer(cols = -Day, names_to = "Simulation", values_to = "Return")

sim_vol_tidy <- as.data.frame(simulated_volatility[, , 1]) %>%  # Assuming 1 regime
  mutate(Day = 1:n_ahead) %>%
  pivot_longer(cols = -Day, names_to = "Simulation", values_to = "Volatility")

sim_cumulative_returns <- apply(simulated_returns, 2, function(x) {
  exp(cumsum(x)) - 1  # Convert log returns to cumulative returns
})

# Convert to tidy format for ggplot
sim_cum_returns_tidy <- as.data.frame(sim_cumulative_returns) %>%
  mutate(Day = 1:n_ahead) %>%
  pivot_longer(cols = -Day, names_to = "Simulation", values_to = "CumulativeReturn")
```

```{r}
ggplot(sim_returns_tidy, aes(x = Day, y = Return, group = Simulation)) +
  geom_line(alpha = 0.3, color = "blue") +
  labs(
    title = "Simulated Return Paths",
    x = "Days Ahead",
    y = "Returns"
  ) +
  theme_minimal()
...
```{r}
ggplot(sim_vol_tidy, aes(x = Day, y = Volatility, group = Simulation)) +
  geom_line(alpha = 0.3, color = "red") +
  labs(
    title = "Simulated Volatility Paths",
    x = "Days Ahead",
    y = "Volatility"
  ) +
  theme_minimal()
...
sim_summary <- sim_returns_tidy %>%
  group_by(Day) %>%
  summarize(
    Mean = mean(Return),
    Lower = quantile(Return, 0.05),
    Upper = quantile(Return, 0.95)
  )

# Calculate summary statistics for volatility
vol_summary <- sim_vol_tidy %>%
  group_by(Day) %>%
  summarize(
    Mean = mean(Volatility),
    Lower = quantile(Volatility, 0.05),
    Upper = quantile(Volatility, 0.95)
  )

cum_summary <- sim_cum_returns_tidy %>%
  group_by(Day) %>%
  summarize(
    Mean = mean(CumulativeReturn),
    Lower = quantile(CumulativeReturn, 0.05),  # 5th percentile
    Upper = quantile(CumulativeReturn, 0.95)   # 95th percentile
  )
...
```{r}
ggplot() +
  geom_line(data = sim_cum_returns_tidy, aes(x = Day, y = CumulativeReturn, group = Simulation), 
            alpha = 0.2, color = "green") +
  geom_line(data = cum_summary, aes(x = Day, y = Mean), color = "black", size = 1) +
  geom_ribbon(data = cum_summary, aes(x = Day, ymin = Lower, ymax = Upper), 
              fill = "green", alpha = 0.2) +
  labs(
    title = "Simulated Cumulative Returns with Mean and 90% Confidence Interval",
    x = "Days Ahead",
...
```{r}
ggplot() +
  geom_line(data = sim_vol_tidy, aes(x = Day, y = Volatility, group = Simulation), 
            alpha = 0.2, color = "red") +
  geom_line(data = vol_summary, aes(x = Day, y = Mean), color = "black", size = 1) +
  geom_ribbon(data = vol_summary, aes(x = Day, ymin = Lower, ymax = Upper), 
              fill = "red", alpha = 0.2) +
  labs(
    title = "Simulated Volatility with Mean and 90% Confidence Interval",
    x = "Days Ahead",
    y = "Volatility"
  ) +
  theme_minimal()
...
Now let's fit our model on the simulated data

```{r}
simulated_results <- list()


sim_separated <- split(sim_returns_tidy, sim_returns_tidy$Simulation)
# Initialize a list to store out-of-sample volatility forecasts for each simulation
simulated_vol_forecasts <- vector("list", length = n_sim)

for (sim_idx in seq_along(sim_separated)) {
  # Extract the current simulation
  sim_data <- sim_separated[[sim_idx]]
  simulated_path <- sim_data$Return  # Extract returns for the current simulation
  
  # Prepare to store out-of-sample volatility forecasts
  out_of_sample_vol_sim <- rep(NA, nrow(sim_data))
  
  # Perform rolling GARCH forecasting
  for (i in seq(window_size + 1, nrow(sim_data))) {
    rolling_window <- simulated_path[(i - window_size + 1):i]
    ...
    if (!is.na(fit_sim)) {
      forecast <- tryCatch({
        predict(fit_sim, n.ahead = 1)
      }, error = function(e) {
        list(vol = 0)  # If forecasting fails, set volatility to 0
      })
      
      out_of_sample_vol_sim[i] <- forecast$vol[1]
    } else {
      out_of_sample_vol_sim[i] <- 0
    }
  }
  
  # Replace NAs with the last observed value
  out_of_sample_vol_sim <- zoo::na.locf(out_of_sample_vol_sim, na.rm = FALSE)
  
  # Append the forecasted volatility to the simulation tibble
  sim_data <- sim_data %>%
    mutate(Forecasted_Volatility = out_of_sample_vol_sim)
  
  # Store the updated simulation tibble in the results list
  simulated_results[[sim_idx]] <- sim_data
}
```

```{r}
simulated_results <- lapply(simulated_results, function(df) df %>% filter(complete.cases(.)))


# Define parameters
target_vols <- c(0.17 / sqrt(252))  # Target volatility (daily terms)
threshold <- 1  
leverage_power <- 20  # Power for leverage calculation
initial_investment <- 100  # Initial investment amount

# Initialize result list for simulations
simulated_strategy_results <- list()

# Process each simulation
for (sim_idx in seq_along(simulated_results)) {
  sim_data <- simulated_results[[sim_idx]]  # Extract the current simulation
  
  # Add doubled returns
  sim_data <- sim_data %>%
    mutate(Doubled_Return = 2 * Return)
  
  # Initialize results for this simulation
  simulation_results <- list()
  
  # Apply the strategy for each target volatility
  for (V_target in target_vols) {
    sim_strategy <- sim_data %>%
      mutate(
        # Calculate today's leverage based on forecasted volatility
        Leverage = pmin((V_target / Forecasted_Volatility)^leverage_power, 3),
        ...
        Portfolio_Return = Leverage_Adjusted * Doubled_Return,
        
        # Cumulative portfolio value
        Cumulative_Portfolio = initial_investment * cumprod(1 + Portfolio_Return) - 1
      ) %>%
      filter(!is.na(Portfolio_Return))  # Remove rows with NA returns due to lagging
    
    # Store the results for this target volatility
    simulation_results[[paste0("Vol_", V_target)]] <- sim_strategy %>%
      select(Day, Cumulative_Portfolio, Leverage, Portfolio_Return)
  }
  
  # Save all results for this simulation
  simulated_strategy_results[[sim_idx]] <- simulation_results
}

# Combine all results into a single data frame for visualization
combined_strategy_results <- do.call(rbind, lapply(seq_along(simulated_strategy_results), function(sim_idx) {
  bind_rows(simulated_strategy_results[[sim_idx]], .id = "Volatility_Target") %>%
    mutate(Simulation = paste0("Sim #", sim_idx))
}))

# Summarize results across simulations
summary_results <- combined_strategy_results %>%
  group_by(Day, Volatility_Target) %>%
  summarize(
    Mean_Cumulative = mean(Cumulative_Portfolio),
    Lower = quantile(Cumulative_Portfolio, 0.05),
    Upper = quantile(Cumulative_Portfolio, 0.95),
    .groups = "drop"
  )
